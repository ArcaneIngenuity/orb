...
static inline void vec3_dup(vec3 a, vec3 const b) //N.W.
{
	int i;
	for(i=0; i<3; ++i)
		a[i] = b[i];
...
static inline void mat4x4_dup(mat4x4 M, mat4x4 N) //N.W.: surely a memcpy would be much faster than 16 conditionals?
{
	int i, j;
	for(i=0; i<4; ++i)
		for(j=0; j<4; ++j)
...
static inline void mat4x4_mul_vec3(vec3 r, mat4x4 M, vec3 v) //N.W. - used for rotating a vec3
{
	int i, j;
	for(j=0; j<3; ++j) {
		r[j] = 0.f;
